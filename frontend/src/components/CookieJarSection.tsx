...
export default function CookieJarSection({
  ...
  const saveJar = async (jar: CookieJar) => {
    ...
    if (cron.split(/\s+/).length !== 5) {
      onSuccess(null)
      onError('Cron must have 5 fields, e.g. 0 */6 * * *.')
      return
    }
        ...
        <div className="space-y-4">
          {jars.map((jar) => {
            ...
            return (
              <div
                key={jar.key}
                className="rounded-lg border border-[var(--bg-300)] dark:border-[var(--bg-300)] bg-[var(--bg-100)] dark:bg-[var(--bg-200)] divide-y divide-[var(--bg-300)] dark:divide-[var(--bg-300)]"
              >
                <div className="flex items-start gap-3 p-4">
                  ...
                  <button
                    ...
                    onClick={() => updateDraft(jar.key, { enabled: !draft.enabled })}
                    disabled={!isAdmin}
                ...
                <div className="flex items-center gap-3 p-4">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-slate-500 dark:text-[var(--text-200)]">Shown in this list. Does not change which fetcher runs.</p>
                  </div>
                  <input
                    type="text"
                    value={draft.name}
                    onChange={(e) => updateDraft(jar.key, { name: e.target.value })}
                    disabled={!isAdmin}
                    maxLength={80}
                ...
                <div className="flex items-center gap-3 p-4">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-slate-500 dark:text-[var(--text-200)]">
                      Five fields in UTC, e.g. 0 */6 * * * (every 6 hours).
                    </p>
                  </div>
                  <input
                    type="text"
                    value={draft.cron}
                    onChange={(e) => updateDraft(jar.key, { cron: e.target.value })}
