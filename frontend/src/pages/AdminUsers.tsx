...
function UserAvatar({ user }: { user: ManagedUser }) {
  ...
  return (
    <span className="w-8 h-8 rounded-full bg-slate-200 dark:bg-gray-700 text-slate-600 dark:text-gray-300 text-xs font-semibold flex items-center justify-center shrink-0 select-none">
...
function Toggle({
  ...
  return (
    <button
      ...
      className={`relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ${
  ...
  return (
    <div className="space-y-6">
      ...
      {!loading && (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {[
            ...
            <div
              key={s.label}
              className="rounded-xl border border-slate-200 dark:border-gray-800 bg-slate-50 dark:bg-gray-900 px-4 py-3"
            >
      ...
      <section className="rounded-xl border border-slate-300/60 dark:border-gray-800 bg-slate-50 dark:bg-gray-900 overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 border-b border-slate-300/60 dark:border-gray-800">
          ...
          <input
            ...
            className="rounded-lg border border-slate-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-1.5 text-sm text-slate-700 dark:text-gray-200 placeholder-slate-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 w-full sm:w-64"
          />
        </div>

        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-slate-100 dark:bg-gray-800/60 text-slate-500 dark:text-gray-400 text-xs uppercase tracking-wide">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-gray-800">
              {loading ? (
                <tr>
                  <td colSpan={4} className="px-5 py-8 text-center text-slate-400 dark:text-gray-500">
                    Loading users…
                  </td>
                </tr>
              ) : filtered.length === 0 ? (
                ...
                filtered.map((u) => (
                  <tr key={u._id} className="hover:bg-slate-100/50 dark:hover:bg-gray-800/40 transition-colors">
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2.5 min-w-0">
                        <UserAvatar user={u} />
                        <div className="min-w-0">
                          <p className="font-medium text-slate-800 dark:text-gray-100 truncate">{u.name}</p>
                    ...
                    <td className="px-5 py-3">
                      {u.role === 'admin' ? (
                        ...
                        <span className="inline-flex items-center rounded-full bg-slate-100 dark:bg-gray-800 text-slate-600 dark:text-gray-400 px-2 py-0.5 text-xs font-medium">
                          User
                        </span>
                      )}
                    </td>
                    <td className="px-5 py-3 text-slate-500 dark:text-gray-400 whitespace-nowrap text-xs">
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center justify-end gap-2.5">
                        {u.role === 'admin' ? (
                          <span className="text-xs text-slate-400 dark:text-gray-500 italic">always on</span>
                        ) : (
                          <>
                            <span className={`text-xs ${u.canCreateLabels ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400 dark:text-gray-500'}`}>
